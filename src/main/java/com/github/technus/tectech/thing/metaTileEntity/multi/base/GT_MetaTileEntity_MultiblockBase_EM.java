...
public abstract class GT_MetaTileEntity_MultiblockBase_EM
    extends GT_MetaTileEntity_MultiBlockBase implements IAlignment {
  ...
  @SideOnly(Side.CLIENT) private SoundLoop activitySoundLoop;
  // endregion

  // region HATCHES ARRAYS - they hold info about found hatches, add hatches to
  // them... (auto structure magic does it tho)
  ...
  protected ArrayList<GT_MetaTileEntity_Hatch_OutputElemental> eOutputHatches =
      new ArrayList<>();
  // EM overflow output
  protected ArrayList<GT_MetaTileEntity_Hatch_OverflowElemental>
      eMufflerHatches = new ArrayList<>();
  // extra hatches
  ...
  // should explode on dismatle?, set it in constructor, if true machine will
  // explode if invalidated structure while active
  protected boolean eDismantleBoom = false;

  // what is the amount of A required
  public long eAmpereFlow = 1; // analogue of EU/t but for amperes used (so eu/t
                               // is actually eu*A/t) USE ONLY POSITIVE NUMBERS!

  // set to what you need it to be in check recipe
  // data required to operate
  protected long eRequiredData = 0;

  // storage for output EM that will be auto handled in case of failure to
  // finish recipe if you succed to use a recipe - be sure to output EM from
  // outputEM to hatches in the output method
  protected EMInstanceStackMap[] outputEM;

  // are parameters correct - change in check recipe/output/update params etc.
  // (maintenance status boolean)
  protected boolean eParameters = true;

  // what type of certainty inconvenience is used - can be used as in Computer -
  // more info in uncertainty hatch
  protected byte eCertainMode = 0, eCertainStatus = 0;

  // minimal repair status to make the machine even usable (how much unfixed
  // fixed stuff is needed) if u need to force some things to be fixed - u might
  // need to override doRandomMaintenanceDamage
  ...
  // max amperes machine can take in after computing it to the lowest tier
  // (exchange packets to min tier count)
  protected long eMaxAmpereFlow = 0, eMaxAmpereGen = 0;

  // What is the max and minimal tier of eu hatches installed
  private long maxEUinputMin = 0, maxEUinputMax = 0, maxEUoutputMin = 0,
               maxEUoutputMax = 0;
  ...
  protected GT_MetaTileEntity_MultiblockBase_EM(int aID, String aName,
  ...
  public void setExtendedFacing(ExtendedFacing newExtendedFacing) {
    if (extendedFacing != newExtendedFacing) {
      ...
      if (getBaseMetaTileEntity().isServerSide()) {
        // NetworkDispatcher.INSTANCE.sendToAllAround(new
        // AlignmentMessage.AlignmentData(this),
  ...
  public void onFacingChange() {
    toolSetDirection(ForgeDirection.getOrientation(
   ...
   */
  public abstract
      IStructureDefinition<? extends GT_MetaTileEntity_MultiblockBase_EM>
      getStructure_EM();

  @SuppressWarnings("unchecked")
  private IStructureDefinition<GT_MetaTileEntity_MultiblockBase_EM>
  getStructure_EM_Internal() {
    return (IStructureDefinition<GT_MetaTileEntity_MultiblockBase_EM>)
        getStructure_EM();
  }

  public final boolean structureCheck_EM(String piece, int horizontalOffset,
                                         int verticalOffset, int depthOffset) {
    IGregTechTileEntity baseMetaTileEntity = getBaseMetaTileEntity();
    return getStructure_EM_Internal().check(
        this, piece, baseMetaTileEntity.getWorld(), getExtendedFacing(),
        baseMetaTileEntity.getXCoord(), baseMetaTileEntity.getYCoord(),
        baseMetaTileEntity.getZCoord(), horizontalOffset, verticalOffset,
        depthOffset, !mMachine);
  }

  public final boolean structureBuild_EM(String piece, int horizontalOffset,
                                         int verticalOffset, int depthOffset,
                                         ItemStack trigger, boolean hintsOnly) {
    IGregTechTileEntity baseMetaTileEntity = getBaseMetaTileEntity();
    return getStructure_EM_Internal().buildOrHints(
        this, trigger, piece, baseMetaTileEntity.getWorld(),
        getExtendedFacing(), baseMetaTileEntity.getXCoord(),
        baseMetaTileEntity.getYCoord(), baseMetaTileEntity.getZCoord(),
   ...
   */
  protected boolean checkMachine_EM(IGregTechTileEntity iGregTechTileEntity,
                                    ItemStack itemStack) {
    return false;
  }
   ...
   */
  public boolean checkRecipe_EM(ItemStack itemStack) { return false; }
   ...
   * (Well it can be overflowed if machine didn't finished,
  ...
  protected void addFluidOutputs(FluidStack[] mOutputFluids) {
    ...
    for (int i = 0; i < min; ++i) {
      if (mOutputHatches.get(i) != null && mOutputFluids[i] != null &&
          GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(
  ...
  public ArrayList<String> getFullLedDescriptionIn(int hatchNo, int paramID) {
    ...
    list.add(
        EnumChatFormatting.WHITE + "Value: " + EnumChatFormatting.AQUA +
  ...
  public ArrayList<String> getFullLedDescriptionOut(int hatchNo, int paramID) {
    ...
    try {
      list.add(
          parametrization.groups[hatchNo].parameterOut[paramID].getBrief());
    } catch (NullPointerException | IndexOutOfBoundsException e) {
      list.add("Unused");
    }
    return list;
  ...
  public String[] getDescription() {
    return new String[] {TEC_MARK_GENERAL, "Nothing special just override me."};
  }
   ...
   * @return
   */
  @Override public String[] getInfoData() { // TODO Do it
    long storedEnergy = 0;
    long maxEnergy = 0;
    ...
    return new String[] {
        ...
        EnumChatFormatting.GREEN + GT_Utility.formatNumbers(storedEnergy) +
            ...
            GT_Utility.formatNumbers(maxEnergy) + EnumChatFormatting.RESET +
            " EU",
        (mEUt * eAmpereFlow <= 0 ? "Probably uses: " : "Probably makes: ") +
            EnumChatFormatting.RED + GT_Utility.formatNumbers(Math.abs(mEUt)) +
            EnumChatFormatting.RESET + " EU/t at " + EnumChatFormatting.RED +
            GT_Utility.formatNumbers(eAmpereFlow) + EnumChatFormatting.RESET +
            " A",
        "Tier Rating: " + EnumChatFormatting.YELLOW +
            VN[getMaxEnergyInputTier_EM()] + EnumChatFormatting.RESET + " / " +
            EnumChatFormatting.GREEN + VN[getMinEnergyInputTier_EM()] +
            EnumChatFormatting.RESET +
            " Amp Rating: " + EnumChatFormatting.GREEN +
        ...
        "Problems: " + EnumChatFormatting.RED +
            ...
            " Efficiency: " + EnumChatFormatting.YELLOW + mEfficiency / 100.0F +
            EnumChatFormatting.RESET + " %",
        "PowerPass: " + EnumChatFormatting.BLUE + ePowerPass +
            EnumChatFormatting.RESET + " SafeVoid: " + EnumChatFormatting.BLUE +
            eSafeVoid,
  ...
  public Object getServerGUI(int aID, InventoryPlayer aPlayerInventory,
    ...
    return new GT_Container_MultiMachineEM(aPlayerInventory,
                                           aBaseMetaTileEntity);
  }
  ...
  public void registerIcons(IIconRegister aBlockIconRegister) {
    ...
    ScreenON =
        new Textures.BlockIcons.CustomIcon("iconsets/EM_CONTROLLER_ACTIVE");
  ...
  protected void soundMagic(ResourceLocation activitySound) {
    if (getBaseMetaTileEntity().isActive()) {
      if (activitySoundLoop == null) {
        activitySoundLoop =
            new SoundLoop(activitySound, getBaseMetaTileEntity(), false, true);
  ...
  public void onRemoval() {
    try {
      if (eOutputHatches != null) {
        for (GT_MetaTileEntity_Hatch_ElementalContainer hatch_elemental :
             eOutputHatches) {
      ...
      cleanOutputEM_EM();
      if (ePowerPass && getEUVar() > V[3] || eDismantleBoom &&
                                                 mMaxProgresstime > 0 &&
                                                 areChunksAroundLoaded_EM()) {
        explodeMultiblock();
      }
    } catch (Exception e) {
      if (DEBUG_MODE) {
  ...
  protected boolean areChunksAroundLoaded_EM() {
    if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(this) &&
      ...
      IGregTechTileEntity base = getBaseMetaTileEntity();
      return base.getWorld().doChunksNearChunkExist(
          base.getXCoord(), base.getYCoord(), base.getZCoord(), 3);
  ...
  /**
   * instantiate parameters in CONSTRUCTOR! CALLED ONCE on creation, don't call
   * it in your classes
   */
   ...
   * Called before check recipe, before outputting, and every second the machine
   * is complete <p> good place to update parameter statuses, default
   * implementation handles it well
   *
   * @param machineBusy is machine doing SHIT
   */
  protected void
  parametersStatusesWrite_EM(boolean machineBusy) { // todo unimplement?
    if (!machineBusy) {
      for (Parameters.Group.ParameterIn parameterIn :
           parametrization.parameterInArrayList) {
        if (parameterIn != null) {
    ...
    for (Parameters.Group.ParameterOut parameterOut :
         parametrization.parameterOutArrayList) {
   ...
   *
   * @param mMachine was the machine considered complete at that point in
   *     onPostTick
   */
  protected void hatchInit_EM(boolean mMachine) {}

  /**
   * called when the multiblock is exploding - if u want to add more EXPLOSIONS,
   * for ex. new types of hatches also have to explode
   */
  protected void extraExplosions_EM() {
  } // For that extra hatches explosions, and maybe some MOORE EXPLOSIONS

  /**
   * Get Available data, Override only on data outputters should return
   * mAvailableData that is set in check recipe
   *
  ...
  protected long getAvailableData_EM() {
    long result = 0;
    IGregTechTileEntity baseMetaTileEntity = getBaseMetaTileEntity();
    Vec3Impl pos = new Vec3Impl(baseMetaTileEntity.getXCoord(),
  ...
  /**
   * Extra hook on cyclic updates (not really needed for machines smaller than 1
   * chunk) BUT NEEDED WHEN - machine blocks are not touching each other or they
   * don't implement IMachineBlockUpdateable (ex. air,stone,weird TE's)
   */
   ...
   */
  public float getExcessMassPerTick_EM(ItemStack itemStack) { return 0f; }

  /**
   * triggered if machine is not allowed to work after completing a recipe,
   * override to make it not shutdown for instance (like turbines). bu just
   * replacing it with blank - active transformer is doing it <p> CALLED
   * DIRECTLY when soft hammered to offline state - usually should stop the
   * machine unless some other mechanics should do it
   */
  protected void notAllowedToWork_stopMachine_EM() { stopMachine(); }
  ...
  public void saveNBTData(NBTTagCompound aNBT) {
    ...
    if (outputEM != null) {
      ...
      for (int i = 0; i < outputEM.length; i++) {
        if (outputEM[i] != null) {
          output.setTag(Integer.toString(i),
                        outputEM[i].toNBT(TecTech.definitionsRegistry));
        }
      }
    ...
    for (int i = 0; i < parametrization.eParamsInStatus.length; i++) {
      paramIs.setByte(Integer.toString(i),
                      parametrization.eParamsInStatus[i].getOrdinalByte());
    }
    ...
    for (int i = 0; i < parametrization.eParamsOutStatus.length; i++) {
      paramOs.setByte(Integer.toString(i),
                      parametrization.eParamsOutStatus[i].getOrdinalByte());
    }
  ...
  public void loadNBTData(NBTTagCompound aNBT) {
    ...
    minRepairStatus =
        aNBT.hasKey("eMinRepair") ? aNBT.getByte("eMinRepair") : 3;
        ...
        if (compound.hasKey(Integer.toString(i))) {
          try {
            outputEM[i] = EMInstanceStackMap.fromNBT(
                ...
                compound.getCompoundTag(Integer.toString(i)));
          } catch (EMException e) {
            if (DEBUG_MODE) {
              e.printStackTrace();
            }
    ...
    if (aNBT.hasKey("eParamsIn") && aNBT.hasKey("eParamsOut") &&
        aNBT.hasKey("eParamsB")) {
      ...
      for (int i = 0; i < 10; i++) {
        if (paramB.getBoolean(Integer.toString(i))) {
          parametrization.iParamsIn[i] =
              Float.intBitsToFloat(paramI.getInteger(Integer.toString(i)));
          parametrization.iParamsOut[i] =
              Float.intBitsToFloat(paramO.getInteger(Integer.toString(i)));
        } else {
          parametrization.iParamsIn[i] = paramI.getInteger(Integer.toString(i));
          parametrization.iParamsOut[i] =
              paramO.getInteger(Integer.toString(i));
        }
      }
    } else {
    ...
    for (int i = 0; i < parametrization.eParamsInStatus.length; i++) {
      parametrization.eParamsInStatus[i] =
          LedStatus.getStatus(paramIs.getByte(Integer.toString(i)));
    }

    NBTTagCompound paramOs = aNBT.getCompoundTag("eParamsOutS");
    for (int i = 0; i < parametrization.eParamsOutStatus.length; i++) {
      parametrization.eParamsOutStatus[i] =
          LedStatus.getStatus(paramOs.getByte(Integer.toString(i)));
    }
  }
   ...
   * On machine stop - NOT called directly when soft hammered to offline state!
   * - it SHOULD cause a full stop like power failure does
   */
  @Override
    ...
    if (outputEM != null) {
      ...
      if (mass > 0) {
        if (eMufflerHatches.size() < 1) {
          ...
          for (GT_MetaTileEntity_Hatch_OverflowElemental dump :
               eMufflerHatches) {
   ...
   */
  @Override
  public final boolean checkMachine(IGregTechTileEntity iGregTechTileEntity,
                                    ItemStack itemStack) {
  ...
  public final boolean checkRecipe(
      ItemStack
          itemStack) { // do recipe checks, based on "machine content and state"
    hatchesStatusUpdate_EM();
    boolean result = checkRecipe_EM(itemStack); // if had no - set default
                                                // params
    hatchesStatusUpdate_EM();
    return result;
  }
  ...
  protected void hatchesStatusUpdate_EM() {
    ...
    if (busy) { // write from buffer to hatches only
      for (GT_MetaTileEntity_Hatch_Param hatch : eParamHatches) {
        ...
        if (parametrization.groups[hatchId] != null &&
            parametrization.groups[hatchId].updateWhileRunning) {
    ...
    if (CERTAIN_MODE) {
      ...
      for (GT_MetaTileEntity_Hatch_Uncertainty uncertainty :
           eUncertainHatches) {
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide()) {
      ...
      if (--mUpdate == 0 || --mStartUpCheck == 0 || cyclicUpdate() ||
        ...
        if (!mMachine) {
          if (ePowerPass && getEUVar() > V[3] ||
              eDismantleBoom && mMaxProgresstime > 0 &&
                  areChunksAroundLoaded_EM()) {
            explodeMultiblock();
          }
          if (outputEM != null) {
            ...
            if (mMaxProgresstime > 0 && doRandomMaintenanceDamage()) { // Start
              if (onRunningTick(mInventory[1])) { // Compute EU
                ...
                if (mMaxProgresstime > 0 &&
                    ++mProgresstime >=
                        mMaxProgresstime) { // progress increase and done
                  hatchesStatusUpdate_EM();
                  ...
                  if (aBaseMetaTileEntity.isAllowedToWork()) {
                    if (checkRecipe(mInventory[1])) {
                      mEfficiency = Math.max(
                          0, min(mEfficiency + mEfficiencyIncrease,
              ...
              } // else {//failed to consume power/resources - inside on running
                // tick
                //    stopMachine();
                //}
            } else if (RECIPE_AT == Tick ||
                       aBaseMetaTileEntity.hasWorkJustBeenEnabled()) {
              if (aBaseMetaTileEntity.isAllowedToWork()) {
                if (checkRecipe(mInventory[1])) {
                  mEfficiency = Math.max(
                      0,
                      min(mEfficiency + mEfficiencyIncrease,
                          getMaxEfficiency(mInventory[1]) -
                ...
                updateSlots();
              } // else notAllowedToWork_stopMachine_EM(); //it is already
                // stopped here
            }
          } else { // not repaired
            stopMachine();
      ...
      aBaseMetaTileEntity.setErrorDisplayID(
          ...
          (mScrewdriver ? 0 : 2) | (mSoftHammer ? 0 : 4) |
          (mHardHammer ? 0 : 8) | (mSolderingTool ? 0 : 16) |
          (mCrowbar ? 0 : 32) | (mMachine ? 0 : 64) |
          (eCertainStatus == 0 ? 0 : 128) | (eParameters ? 0 : 256));
  ...
  protected void safeVoid_EM() {
    for (GT_MetaTileEntity_Hatch_OverflowElemental voider : eMufflerHatches) {
      ...
      for (GT_MetaTileEntity_Hatch_InputElemental in : eInputHatches) {
        for (EMInstanceStack instance :
             in.getContentHandler().valuesToArray()) {
          double qty = div(remaining, instance.getDefinition().getMass());
          if (qty > 0) {
            qty = min(qty, instance.getAmount());
            if (voider.addOverflowMatter(instance.getDefinition().getMass() *
                                         qty)) {
              voider.setOverflowMatter(voider.overflowMax);
            }
            in.getContentHandler().removeAmount(
                new EMDefinitionStack(instance.getDefinition(), qty));
          }
        }
      }
      for (GT_MetaTileEntity_Hatch_OutputElemental out : eOutputHatches) {
        for (EMInstanceStack instance :
             out.getContentHandler().valuesToArray()) {
          double qty = div(remaining, instance.getDefinition().getMass());
          if (qty > 0) {
            qty = min(qty, instance.getAmount());
            if (voider.addOverflowMatter(instance.getDefinition().getMass() *
            ...
            out.getContentHandler().removeAmount(
                new EMDefinitionStack(instance.getDefinition(), qty));
          }
        }
      }
  ...
  protected void maintenance_EM() {
    if (GT_MetaTileEntity_MultiBlockBase.disableMaintenance) {
      ...
      for (GT_MetaTileEntity_Hatch_Maintenance tHatch : mMaintenanceHatches) {
        if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(tHatch)) {
          if (tHatch.mAuto && !(mWrench && mScrewdriver && mSoftHammer &&
                                mHardHammer && mSolderingTool && mCrowbar)) {
  ...
  protected void clearHatches_EM() {
    ...
    for (GT_MetaTileEntity_Hatch_ElementalContainer hatch_elemental :
         eOutputHatches) {
      if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(
              hatch_elemental)) {
        hatch_elemental.id = -1;
      }
    }
  ...
  protected void setupEnergyHatchesVariables_EM() {
    if (!mEnergyHatches.isEmpty() || !eEnergyMulti.isEmpty() ||
        !mDynamoHatches.isEmpty() || !eDynamoMulti.isEmpty()) {
      maxEUinputMin = V[15];
      maxEUinputMax = V[0];
      maxEUoutputMin = V[15];
      maxEUoutputMax = V[0];
  ...
  protected void
  dischargeController_EM(IGregTechTileEntity aBaseMetaTileEntity) {
  ...
  protected final void powerPass(IGregTechTileEntity aBaseMetaTileEntity) {
    ...
    for (GT_MetaTileEntity_Hatch_Dynamo tHatch : mDynamoHatches) {
      if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(tHatch)) {
        ...
        if (tHatch.getBaseMetaTileEntity().getStoredEU() <=
                tHatch.maxEUStore() - euVar &&
            aBaseMetaTileEntity.decreaseStoredEnergyUnits(
                euVar + Math.max(euVar / 24576, tHatch.maxAmperesOut()),
                false)) {
    ...
    for (GT_MetaTileEntity_Hatch_Dynamo tHatch : mDynamoHatches) {
      if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(tHatch)) {
        ...
        if (tHatch.getBaseMetaTileEntity().getStoredEU() <=
                tHatch.maxEUStore() - euVar &&
            aBaseMetaTileEntity.decreaseStoredEnergyUnits(
                euVar + Math.max(euVar / 24576, 1), false)) {
          tHatch.setEUVar(tHatch.getBaseMetaTileEntity().getStoredEU() + euVar);
        }
      }
    }
  ...
  protected final void powerInput() {
    ...
    for (GT_MetaTileEntity_Hatch_Energy tHatch : mEnergyHatches) {
      ...
      if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(tHatch)) {
        euVar = Math.min(tHatch.maxEUInput() * tHatch.maxAmperesIn(),
                         tHatch.getEUVar());
        if (tHatch.getBaseMetaTileEntity().decreaseStoredEnergyUnits(euVar,
                                                                     false)) {
  ...
  public int getRepairStatus() {
    return super.getRepairStatus() + (eCertainStatus == 0 ? 1 : 0) +
        (eParameters ? 1 : 0);
  }
  ...
  // new method
  public boolean energyFlowOnRunningTick_EM(ItemStack aStack,
                                            boolean allowProduction) {
    long euFlow = mEUt * eAmpereFlow; // quick scope sign
    if (allowProduction && euFlow > 0) {
      addEnergyOutput_EM((long)mEUt * (long)mEfficiency /
                             getMaxEfficiency(aStack),
                         eAmpereFlow);
    } else if (euFlow < 0) {
      if (POWERLESS_MODE) {
        return true;
      }
      if (!drainEnergyInput_EM(mEUt,
                               (long)mEUt * getMaxEfficiency(aStack) /
                                   Math.max(1000L, mEfficiency),
                               eAmpereFlow)) {
        criticalStopMachine();
        return false;
      }
  ...
  public boolean energyFlowOnRunningTick(ItemStack aStack,
                                         boolean allowProduction) {
    long euFlow = mEUt * eAmpereFlow; // quick scope sign
    if (allowProduction && euFlow > 0) {
      addEnergyOutput_EM((long)mEUt * (long)mEfficiency /
      ...
      if (!drainEnergyInput((long)mEUt * getMaxEfficiency(aStack) /
                                Math.max(1000L, mEfficiency),
                            eAmpereFlow)) {
        criticalStopMachine();
        return false;
  ...
  // public final boolean energyFlowWithoutEffieciencyComputation(int eu,long
  // ampere) {
  ...
  public long maxEUStore() {
    return Math.max(maxEUinputMin * (eMaxAmpereFlow << 3),
                    maxEUoutputMin * (eMaxAmpereGen << 3));
  }
  ...
  public boolean addEnergyOutput_EM(long EU, long Amperes) {
    ...
    for (GT_MetaTileEntity_Hatch_Dynamo tHatch : mDynamoHatches) {
      if (GT_MetaTileEntity_MultiBlockBase.isValidMetaTileEntity(tHatch)) {
        ...
        diff =
            tHatch.maxEUStore() - tHatch.getBaseMetaTileEntity().getStoredEU();
        if (diff > 0) {
          if (euVar > diff) {
            tHatch.setEUVar(tHatch.maxEUStore());
            euVar -= diff;
          } else if (euVar <= diff) {
            tHatch.setEUVar(tHatch.getBaseMetaTileEntity().getStoredEU() +
                            euVar);
            return true;
          }
        }
  ...
  public boolean drainEnergyInput_EM(long EUtTierVoltage, long EUtEffective,
                                     long Amperes) {
    ...
    if (EUuse > getEUVar() || // not enough power
        (EUtTierVoltage == 0
             ...
             : (EUtTierVoltage >
                maxEUinputMax) || // TIER IS BASED ON BEST HATCH! not total
                                  // EUtEffective input
                   (EUtTierVoltage * Amperes - 1) / maxEUinputMin + 1 >
                       eMaxAmpereFlow)) { // EUuse==0? --> (EUuse - 1) /
                                          // maxEUinputMin + 1 = 1! //if not too
                                          // much A
      if (DEBUG_MODE) {
        TecTech.LOGGER.debug("L1 " + EUuse + ' ' + getEUVar() + ' ' +
                             (EUuse > getEUVar()));
        TecTech.LOGGER.debug("L2 " + EUtEffective + ' ' + maxEUinputMax + ' ' +
                             (EUtEffective > maxEUinputMax));
        TecTech.LOGGER.debug("L3 " + Amperes + ' ' + getMaxInputEnergy());
        TecTech.LOGGER.debug(
            "L4 " + ((EUuse - 1) / maxEUinputMin + 1) + ' ' + eMaxAmpereFlow +
  ...
  // new method
  public final boolean overclockAndPutValuesIn_EM(long EU,
                                                  int time) { // TODO revise
    if (EU == 0L) {
      mEUt = 0;
    ...
    while (tempEUt < tempTier) {
      ...
      time >>= 1;
      EU =
          time == 0 ? EU >> 1 : EU << 2; // U know, if the time is less than 1
                                         // tick make the machine use less power
    }
  ...
  // new Method
  public final int getMaxEnergyInputTier_EM() { return getTier(maxEUinputMax); }

  // new Method
  public final int getMinEnergyInputTier_EM() { return getTier(maxEUinputMin); }
  ...
  public void
  cleanHatchContentEM_EM(GT_MetaTileEntity_Hatch_ElementalContainer target) {
    if (target == null) {
      return;
    }
  ...
  public final void explodeMultiblock() {
    ...
    if (!TecTech.configTecTech.BOOM_ENABLE) {
      TecTech.proxy.broadcast("Multi Explode BOOM! " +
  ...
  public void doExplosion(long aExplosionPower) {
    if (!TecTech.configTecTech.BOOM_ENABLE) {
      TecTech.proxy.broadcast("Multi DoExplosion BOOM! " +
                              getBaseMetaTileEntity().getXCoord() + ' ' +
                              getBaseMetaTileEntity().getYCoord() + ' ' +
                              getBaseMetaTileEntity().getZCoord());
      StackTraceElement[] ste = Thread.currentThread().getStackTrace();
  ...
  @Override
  public final boolean addToMachineList(IGregTechTileEntity aTileEntity,
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_InputBus) {
      return mInputBusses.add(
          (GT_MetaTileEntity_Hatch_InputBus)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Output) {
      return mOutputHatches.add(
          (GT_MetaTileEntity_Hatch_Output)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_OutputBus) {
      return mOutputBusses.add(
          (GT_MetaTileEntity_Hatch_OutputBus)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Energy) {
      return mEnergyHatches.add(
          (GT_MetaTileEntity_Hatch_Energy)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Dynamo) {
      return mDynamoHatches.add(
          (GT_MetaTileEntity_Hatch_Dynamo)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Maintenance) {
      return mMaintenanceHatches.add(
          (GT_MetaTileEntity_Hatch_Maintenance)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Muffler) {
      return mMufflerHatches.add(
          (GT_MetaTileEntity_Hatch_Muffler)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_InputElemental) {
      return eInputHatches.add(
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_OutputElemental) {
      return eOutputHatches.add(
          (GT_MetaTileEntity_Hatch_OutputElemental)aMetaTileEntity);
    }
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Uncertainty) {
      return eUncertainHatches.add(
          (GT_MetaTileEntity_Hatch_Uncertainty)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_OverflowElemental) {
      return eMufflerHatches.add(
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_EnergyMulti) {
      return eEnergyMulti.add(
          (GT_MetaTileEntity_Hatch_EnergyMulti)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_DynamoMulti) {
      return eDynamoMulti.add(
          (GT_MetaTileEntity_Hatch_DynamoMulti)aMetaTileEntity);
    }
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_OutputData) {
      return eOutputData.add(
          (GT_MetaTileEntity_Hatch_OutputData)aMetaTileEntity);
    }
    return false;
  }

  public final boolean addClassicToMachineList(IGregTechTileEntity aTileEntity,
                                               int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  public final boolean addElementalToMachineList(
  ...
  public final boolean addClassicMufflerToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  public final boolean addElementalMufflerToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  @Override
  public final boolean addMufflerToMachineList(IGregTechTileEntity aTileEntity,
                                               int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  @Override
  public final boolean addInputToMachineList(IGregTechTileEntity aTileEntity,
                                             int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
    ...
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Input) {
      ((GT_MetaTileEntity_Hatch)aMetaTileEntity)
          .updateTexture(aBaseCasingIndex);
      ((GT_MetaTileEntity_Hatch_Input)aMetaTileEntity).mRecipeMap =
          getRecipeMap();
      return mInputHatches.add((GT_MetaTileEntity_Hatch_Input)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_InputBus) {
      ((GT_MetaTileEntity_Hatch)aMetaTileEntity)
          .updateTexture(aBaseCasingIndex);
      ((GT_MetaTileEntity_Hatch_InputBus)aMetaTileEntity).mRecipeMap =
          getRecipeMap();
      return mInputBusses.add(
          (GT_MetaTileEntity_Hatch_InputBus)aMetaTileEntity);
    }
    if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_InputElemental) {
      ((GT_MetaTileEntity_Hatch)aMetaTileEntity)
          .updateTexture(aBaseCasingIndex);
      //((GT_MetaTileEntity_Hatch_Elemental) aMetaTileEntity).mRecipeMap =
      //getRecipeMap();
      return eInputHatches.add(
          (GT_MetaTileEntity_Hatch_InputElemental)aMetaTileEntity);
    }
  ...
  @Override
  public final boolean addOutputToMachineList(IGregTechTileEntity aTileEntity,
                                              int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  @Deprecated
  @Override
  public final boolean addEnergyInputToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  @Deprecated
  @Override
  public final boolean addDynamoToMachineList(IGregTechTileEntity aTileEntity,
                                              int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // New Method
  public final boolean addEnergyIOToMachineList(IGregTechTileEntity aTileEntity,
                                                int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addElementalInputToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addElementalOutputToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addClassicInputToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addClassicOutputToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addParametrizerToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addUncertainToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  @Override
  public final boolean addMaintenanceToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addClassicMaintenanceToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
  ...
  // NEW METHOD
  public final boolean addDataConnectorToMachineList(
      IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
    if (aTileEntity == null) {
      return false;
    }
