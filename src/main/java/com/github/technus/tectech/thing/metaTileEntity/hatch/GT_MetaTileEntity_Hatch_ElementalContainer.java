...
public abstract class GT_MetaTileEntity_Hatch_ElementalContainer
  ...
  protected GT_MetaTileEntity_Hatch_ElementalContainer(int aID, String aName,
  ...
  public void registerIcons(IIconRegister aBlockIconRegister) {
    ...
    EM_T_ACTIVE =
        new Textures.BlockIcons.CustomIcon("iconsets/OVERLAY_EM_T_ACTIVE");
    EM_T_SIDES =
        new Textures.BlockIcons.CustomIcon("iconsets/OVERLAY_EM_T_SIDES");
  ...
  public ITexture[] getTexturesActive(ITexture aBaseTexture) {
    return new ITexture[] {
        ...
        new GT_RenderedTexture(
            EM_T_ACTIVE,
            Dyes.getModulation(getBaseMetaTileEntity().getColorization(),
                               MACHINE_METAL.getRGBA())),
  ...
  public void loadNBTData(NBTTagCompound aNBT) {
    ...
    try {
      content = EMInstanceStackMap.fromNBT(TecTech.definitionsRegistry,
                                           aNBT.getCompoundTag("eM_Stacks"));
    } catch (EMException e) {
      if (DEBUG_MODE) {
        e.printStackTrace();
      }
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide()) {
      ...
      if (DECAY_AT == Tick) {
        purgeOverflow();
        content.tickContent(1, postEnergize,
                            1); // Hatches don't life time mult things
        purgeOverflow();
        ...
        if (overflowMatter <= 0) {
          ...
          if (deathDelay == 1) {
            IGregTechTileEntity tGTTileEntity =
                aBaseMetaTileEntity.getIGregTechTileEntityAtSide(
                    aBaseMetaTileEntity.getBackFacing());
            if (tGTTileEntity == null ||
                !(tGTTileEntity.getMetaTileEntity() instanceof
                  GT_MetaTileEntity_Hatch_OverflowElemental)) {
              tGTTileEntity =
                  aBaseMetaTileEntity.getIGregTechTileEntityAtSide((byte)0);
            }
            if (tGTTileEntity == null ||
                !(tGTTileEntity.getMetaTileEntity() instanceof
                  GT_MetaTileEntity_Hatch_OverflowElemental)) {
              tGTTileEntity =
                  aBaseMetaTileEntity.getIGregTechTileEntityAtSide((byte)1);
            }
            if (tGTTileEntity != null &&
                tGTTileEntity.getMetaTileEntity() instanceof
                    GT_MetaTileEntity_Hatch_OverflowElemental) {
              ...
              if (aMetaTileEntity.addOverflowMatter(overflowMatter)) {
                if (TecTech.configTecTech.BOOM_ENABLE) {
                  ...
                  TecTech.anomalyHandler.addAnomaly(aBaseMetaTileEntity,
                                                    overflowMatter * 32D);
              ...
              deathDelay = 3; // needed in some cases like repetitive failures.
                              // Should be 4 since there is -- at end but meh...
              overflowMatter = 0F;
            }
            ...
            if (TecTech.configTecTech.BOOM_ENABLE) {
              ...
              deathDelay = 3;
              overflowMatter = 0;
              TecTech.proxy.broadcast("Container0 " +
  ...
  public int getMaxStacksCount() { return mTier * mTier >> 4; }
  ...
  public void purgeOverflow() {
    overflowMatter +=
        content.removeOverflow(getMaxStacksCount(), getMaxStackSize());
  }
  ...
  public boolean onRightclick(IGregTechTileEntity aBaseMetaTileEntity,
    ...
    if (!aBaseMetaTileEntity.isClientSide() && aPlayer instanceof
                                                   EntityPlayerMP) {
      try {
    ...
    if (TecTech.configTecTech.EASY_SCAN || DEBUG_MODE) {
      if (id > 0) {
        if (content == null || content.size() == 0) {
          return new String[] {
              ...
              translateToLocalFormatted("tt.keyphrase.No_Stacks",
                                        clientLocale)};
        } else {
      ...
      if (id > 0) {
        ...
        return new String[] {
            translateToLocalFormatted("tt.keyword.ID", clientLocale) + ": " +
                EnumChatFormatting.AQUA + id,
            translateToLocalFormatted("tt.keyphrase.Contains_EM",
                                      clientLocale)};
      }
      if (content == null || content.size() == 0) {
        return new String[] {
  ...
  public void updateSlots() { purgeOverflow(); }
  ...
  public String[] getDescription() {
    return new String[] {
        ...
        translateToLocal("tt.base.emhatch.desc.1") + " " +
            EnumChatFormatting.AQUA +
            TT_Utility.formatNumberShortExp(
                getMaxStackSize()),                 // Stack capacity:
        translateToLocal("tt.base.emhatch.desc.2"), // Place Overflow Hatch
                                                    // behind,on top or below
        translateToLocal(
            "tt.base.emhatch.desc.3"), // to provide overflow protection while
                                       // this block
        translateToLocal(
            "tt.base.emhatch.desc.4"), // is not attached to multi block.
        translateToLocal("tt.base.emhatch.desc.5"), // Transport range can be
                                                    // extended in straight
        translateToLocal("tt.base.emhatch.desc.6"), // line up to 15 blocks with
                                                    // quantum tunnels.
        EnumChatFormatting.AQUA +
            translateToLocal(
  ...
  public void onRemoval() {
    if (isValidMetaTileEntity(this) && getBaseMetaTileEntity().isActive()) {
      TecTech.anomalyHandler.addAnomaly(
          getBaseMetaTileEntity(), (overflowMatter + content.getMass()) * 16D);
      IGregTechTileEntity base = getBaseMetaTileEntity();
      if (TecTech.configTecTech.BOOM_ENABLE) {
        base.doExplosion(V[15]);
      } else {
        TecTech.proxy.broadcast(translateToLocal("tt.keyword.BOOM") + " " +
