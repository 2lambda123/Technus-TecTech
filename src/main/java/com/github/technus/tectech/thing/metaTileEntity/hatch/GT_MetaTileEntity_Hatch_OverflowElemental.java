...
public class GT_MetaTileEntity_Hatch_OverflowElemental
  ...
  public GT_MetaTileEntity_Hatch_OverflowElemental(int aID, String aName,
                                                   String aNameRegional,
                                                   int aTier, double max) {
    super(aID, aName, aNameRegional, aTier, 0,
          translateToLocal(
              "gt.blockmachines.hatch.emmuffler.desc.0")); // Disposes excess
                                                           // elemental Matter
  ...
  public GT_MetaTileEntity_Hatch_OverflowElemental(String aName, int aTier,
                                                   double max,
                                                   String aDescription,
                                                   ITexture[][][] aTextures) {
  ...
  public void registerIcons(IIconRegister aBlockIconRegister) {
    ...
    MufflerEMidle =
        new Textures.BlockIcons.CustomIcon("iconsets/MUFFLER_EM_IDLE");
  }
  ...
  public String[] getDescription() {
    return new String[] {
        CommonValues.TEC_MARK_EM, mDescription,
  ...
  public MetaTileEntity newMetaEntity(IGregTechTileEntity aTileEntity) {
    return new GT_MetaTileEntity_Hatch_OverflowElemental(
        mName, mTier, overflowMax, mDescription, mTextures);
  }
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide() && aTick % 20 == DISPERSE_AT) {
      if (aBaseMetaTileEntity.isActive()) {
        if (overflowMatter > overflowDisperse) {
          TecTech.anomalyHandler.addAnomaly(
              aBaseMetaTileEntity, overflowDisperse / (Math.pow(2, mTier)));
          overflowMatter -= overflowDisperse;
        } else {
          TecTech.anomalyHandler.addAnomaly(
              aBaseMetaTileEntity, overflowMatter / (Math.pow(2, mTier)));
          overflowMatter = 0;
          aBaseMetaTileEntity.setActive(false);
          aBaseMetaTileEntity.setLightValue((byte)0);
          aBaseMetaTileEntity.getWorld().updateLightByType(
              EnumSkyBlock.Block, aBaseMetaTileEntity.getXCoord(),
      ...
      TecTech.proxy.em_particle(aBaseMetaTileEntity,
                                aBaseMetaTileEntity.getFrontFacing());
    }
    super.onPostTick(aBaseMetaTileEntity, aTick);
    // DOES NOT CHECK FOR TOO MUCH, it is done only while putting stuff in
    // (OPTIMIZATION!!!)
  }
  ...
  private void vapePollution(IGregTechTileEntity mte) {
    ...
    AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(
        xPos - .5 + xDirShift, yPos - .5 + yDirShift, zPos - .5 + zDirShift,
        xPos + .5 + xDirShift, yPos + 1.5 + yDirShift, zPos + .5 + zDirShift);
    for (Object entity :
         mte.getWorld().getEntitiesWithinAABB(EntityLivingBase.class, aabb)) {
      float damagingFactor = (float)Math.log(overflowDisperse);
      ((EntityLivingBase)entity)
          .addPotionEffect(new PotionEffect(Potion.confusion.id, 1,
                                            (int)(damagingFactor * 20)));
      ((EntityLivingBase)entity)
          .addPotionEffect(new PotionEffect(Potion.wither.id, 2,
                                            (int)(damagingFactor * 15)));
      ((EntityLivingBase)entity)
          .attackEntityFrom(elementalPollution, damagingFactor);
  ...
  public String[] getInfoData() {
    return new String[] {
        ...
        EnumChatFormatting.GREEN +
            ...
            EnumChatFormatting.RESET + " " +
            translateToLocal("tt.keyword.unit.mass"),
        translateToLocalFormatted("tt.keyphrase.Mass_Disposal_speed",
                                  clientLocale) +
  ...
  public void onRemoval() {
    if (isValidMetaTileEntity(this) && getBaseMetaTileEntity().isActive()) {
      ...
      if (TecTech.configTecTech.BOOM_ENABLE) {
        ...
        TecTech.proxy.broadcast(translateToLocalFormatted(
                                    "tt.keyphrase.Muffler_BOOM", clientLocale) +
  ...
  public double getOverflowMatter() { return overflowMatter; }
